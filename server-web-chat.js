...
io.on('connection', function (socket) {
	...
	socket.on("request online users", function (requester) {
		//Don't want to display the requester to themselves, so we remove it server side
		var userlist_sans_request = userlist.slice();

		var index = userlist_sans_request.map(function (x) {
			return x.name;
		}).indexOf(requester);
		if (index != -1) {
	...
	socket.on("typing", function (user, target) {
		if (target == "Lobby") {
	...
	socket.on('disconnect', function () {
		...
		var index = userlist.findIndex(function (x) {
			return x.name == userid;
		});
		if (index != -1) {
...
function getGravatar(username) {
	var index = userlist.map(function (x) {
		return x.name;
	}).indexOf(username);
	if (index != -1) return userlist[index].gravatar;
}
...
function usernameToSocket(username) {
	...
	if (index != -1) return userlist[index].socket;
}
...
function socketToUsername() {
	...
	if (index != -1) return userlist[index].name;
}
